...
An interactive interface designed for contactless full-body interaction with audio and video in a web browser. More details about this project can be found at the [MMT Website](https://www.mmtshow.com/projects/Tanay-Chowdhury).
...
1. Pull the repo
2. In the `/MMT-Project/Posenet-Max-p5` folder open a terminal window and run `npm install` (note: Node must be installed prior to this step). This will install all the dependencies related to the project.
...
```
WEBCAM → PoseNet (ml5.js) → 17 body keypoints
                    ↓
             sketch.js (p5.js)
             ├── Calculate: body tilt, center of gravity
             ├── Render: cardioid, particles, flow field, skeleton
             └── Socket.io → server.js (Node/Express)
                                ↓               ↓
                            max-api          OSC/UDP
                                ↓               ↓
                            Max/MSP ←── Wekinator (5 ML models)
                                ↓
                        AUDIO (Beap synth, 3 voices)
```
